...
## 2025-02-12 - [Ensure Imports with useShallow]
**Learning:** When adding `useShallow` for Zustand selectors, it's easy to forget to add the import `import { useShallow } from 'zustand/react/shallow'` especially if the file previously had individual selectors. Forgetting this causes a `ReferenceError` at runtime.
**Action:** Always verify that `useShallow` is imported when refactoring Zustand selectors.
