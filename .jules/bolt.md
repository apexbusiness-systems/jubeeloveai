...
## 2025-02-12 - [Batch Queueing Optimization]
**Learning:** In sync failure scenarios, looping and queueing items individually for retry is inefficient and causes performance bottlenecks (O(N) db ops).
**Action:** When a batch database operation fails due to transient network errors, enqueue the entire batch payload at once using a `sync_batch` operation type, rather than splitting it up. When processing the queue, unpack the `batch` and use single-call database `.upsert(batchData)` and IndexedDB `.putBulk` for performance.
