...
## 2025-02-13 - [Zustand useShallow Grouping]
**Learning:** Found additional files (`ColorGame.tsx`, `SessionMonitor.tsx`, `DevAuthOverride.tsx`) with multiple single-field Zustand selectors that were missed in previous optimizations. Combining them with `useShallow` consistently across the codebase ensures maximum performance and reduces memory allocations.
**Action:** Always group related Zustand selectors into a single object and wrap them with `useShallow` to reduce store subscriptions and unnecessary re-renders.
