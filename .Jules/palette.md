...
## 2024-05-30 - Missing ARIA label on Story Time interactive page
**Learning:** Found that the main interactive page element in the Story Time module had `role="button"` and `tabIndex={0}` but lacked an `aria-label`, leaving screen readers without context for what the interaction did.
**Action:** Always ensure that any element acting as a button (`role="button"`) has an appropriate, dynamic `aria-label` (e.g., changing from "Next page" to "Finish story" based on state) to guide screen reader users.
