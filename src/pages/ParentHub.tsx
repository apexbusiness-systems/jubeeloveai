...
// ⚡ Bolt: Precompute skill map for O(1) lookup instead of O(n) array find per skill iteration
const SKILLS_MAP = Object.values(Skills).reduce((acc, skill) => {
  acc[skill.id] = skill;
  return acc;
                          ...
                          <div className="space-y-2 text-xs">
                            <div>
                              ...
                              {strongest.length > 0 ? (
                                <ul className="text-muted-foreground list-disc list-inside">
                                  {strongest.map(s => <li key={s.skillId}>{SKILLS_MAP[s.skillId]?.name || s.skillId}</li>)}
                                </ul>
                              ) : (
                            ...
                            <div>
                              ...
                              {needsReview.length > 0 ? (
                                <ul className="text-muted-foreground list-disc list-inside">
                                  {needsReview.map(s => <li key={s.skillId}>{SKILLS_MAP[s.skillId]?.name || s.skillId}</li>)}
                                </ul>
                              ) : (
