...
export function useJubeeLifecycleDiagnostics(containerRef: React.RefObject<HTMLDivElement>) {
  ...
  const { isVisible, containerPosition, position, currentAnimation, isDragging } = useJubeeStore(useShallow(state => ({ isVisible: state.isVisible, containerPosition: state.containerPosition, position: state.position, currentAnimation: state.currentAnimation, isDragging: state.isDragging })));
