  ...
  useFrame((state) => {
    ...
    try {
      ...
      const shouldSkipFrame =
        performanceProfile?.quality === "low" && state.frameloop !== "never";
      ...
      if (leftWingRef.current) {
        ...
        leftWingRef.current.rotation.y +=
          (targetFlapLeft - leftWingRef.current.rotation.y) * 0.3;
      }
      if (rightWingRef.current) {
        rightWingRef.current.rotation.y +=
          (targetFlapRight - rightWingRef.current.rotation.y) * 0.3;
      }
      ...
      if (leftAntennaRef.current && rightAntennaRef.current) {
        ...
        const antennaSpeed =
          currentMood === "excited" ? 4 : currentMood === "tired" ? 0.5 : 2;

        // Left antenna sways
        leftAntennaRef.current.rotation.z =
          Math.sin(time * antennaSpeed) * antennaWave;
        leftAntennaRef.current.rotation.x =
          Math.cos(time * antennaSpeed * 0.7) * antennaWave * 0.5;

        // Right antenna sways (opposite phase)
        rightAntennaRef.current.rotation.z =
          -Math.sin(time * antennaSpeed) * antennaWave;
        rightAntennaRef.current.rotation.x =
          Math.cos(time * antennaSpeed * 0.7) * antennaWave * 0.5;
      }
  ...
  return (
    ...
    >
      ...
      <mesh position={[0, 0.15, 0]} castShadow>
        <sphereGeometry
          args={[0.51, segments, segments, 0, Math.PI * 2, 0.7, 0.3]}
        />
        <meshStandardMaterial
      ...
      <mesh position={[0, -0.15, 0]} castShadow>
        <sphereGeometry
          args={[0.51, segments, segments, 0, Math.PI * 2, 1.3, 0.3]}
        />
        <meshStandardMaterial
          color={currentColors.stripe}
          roughness={0.3}
      ...
      <mesh position={[0, -0.3, 0]} castShadow>
        <sphereGeometry
          args={[0.52, segments, segments, 0, Math.PI * 2, 1.6, 0.2]}
        />
        <meshStandardMaterial
      ...
      >
        <torusGeometry
          args={[
            faceExpression.smileRadius,
            0.02,
            16,
            48,
            faceExpression.smileArc,
          ]}
        />
        <meshStandardMaterial color={currentColors.eyePupil} roughness={0.5} />
      </a.mesh>
      ...
      <group>
        ...
        <mesh ref={leftAntennaRef} position={[-0.15, 0.95, 0]} castShadow>
          ...
          <meshStandardMaterial color={currentColors.stripe} roughness={0.4} />
        </mesh>
      ...
      <Sparkles
        count={
          currentMood === "excited" ? 40 : currentMood === "tired" ? 8 : 20
        }
        scale={
          currentMood === "excited" ? 2.5 : currentMood === "tired" ? 1.5 : 2
        }
        size={currentMood === "excited" ? 4 : currentMood === "tired" ? 2 : 3}
        speed={
          currentMood === "excited" ? 0.6 : currentMood === "tired" ? 0.1 : 0.3
        }
        opacity={
          currentMood === "excited" ? 0.8 : currentMood === "tired" ? 0.3 : 0.6
        }
        color={currentColors.accent}
      />
      ...
      <spotLight
        ...
        penumbra={1}
        intensity={
          currentMood === "excited" ? 1.8 : currentMood === "tired" ? 0.6 : 1.2
        }
