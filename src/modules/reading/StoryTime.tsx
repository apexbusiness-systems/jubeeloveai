...
interface AudioControlsProps {
  ...
  onPauseResume: (e: React.MouseEvent<HTMLButtonElement> | React.TouchEvent<HTMLButtonElement>) => void
...
export default function StoryTime() {
  ...
  useEffect(() => {
    const fetchStories = async () => {
      try {
        ...
        // Transform Supabase data to Story interface safely
        // We use explicit casting for 'pages' because Supabase returns Json type
        ...
        // Fallback to local hardcoded data on error
  ...
  const handleStorySelect = useCallback((story: Story) => {
    ...
    // Performance: Preload assets
    preloadStoryContext(story.pages)
    
    // Auto-play initial narration
    setTimeout(() => {
      setIsNarrating(true)
      setIsPaused(false)
  ...
  }, [speak, triggerAnimation, preloadStoryContext, playbackSpeed])

  const handleNextPage = useCallback((e: React.MouseEvent | React.TouchEvent) => {
    e.preventDefault()
    e.stopPropagation()
  ...
  }, [selectedStory, currentPage, speak, triggerAnimation, addScore, playbackSpeed])

  const handlePrevPage = useCallback((e: React.MouseEvent | React.TouchEvent) => {
    e.preventDefault()
    e.stopPropagation()
    
    if (!selectedStory || currentPage <= 0) return
    ...
    const duration = (words / 150) * 60 * 1000 / playbackSpeed
    setTimeout(() => setIsNarrating(false), duration)
  }, [selectedStory, currentPage, speak, triggerAnimation, playbackSpeed])

  const handleReadAloud = useCallback((e: React.MouseEvent | React.TouchEvent) => {
    e.preventDefault()
    e.stopPropagation()
  ...
  const handlePauseResume = useCallback((e: React.MouseEvent<HTMLButtonElement> | React.TouchEvent<HTMLButtonElement>) => {
    e.preventDefault()
    e.stopPropagation()
